...
def save_safeloras_for_webui(  # made by tbq
    ...
    def extract_lora_tensor(
            model, target_replace_module=DEFAULT_TARGET_REPLACE, as_fp16=True, prefix='lora_te'
    ):
            ...
            if module.__class__.__name__ in target_replace_module:
                for child_name, child_module in module.named_modules():
                    if child_module.__class__.__name__ == 'LoraInjectedLinear'or (
                            child_module.__class__.__name__ == "LoraInjectedConv2d" and child_module.kernel_size == (1, 1)):
    ...
    for name, (model, target_replace_module) in modelmap.items():
        metadata[name] = json.dumps(list(target_replace_module))
        if name == 'unet':
            weights_tmp = extract_lora_tensor(model, target_replace_module, prefix='lora_unet')
        else:
            weights_tmp = extract_lora_tensor(model, target_replace_module, prefix='lora_te')
